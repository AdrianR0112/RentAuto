...
function getExpandedStats($dbh)
{
    ...
    // Total de usuarios
    $stmt = $dbh->query("SELECT COUNT(*) as total FROM admin");
    ...
    // Ingresos totales (necesitaría un campo de precio que no está en el esquema)
    ...
    // No hay información de estados de vehículos en el esquema
    $stats['available_vehicles'] = 0;
    $stats['maintenance_vehicles'] = 0;
    $stats['rented_vehicles'] = 0;
...
function getChartData($dbh)
{
    ...
    // No hay campo de precio por día, así que los ingresos mensuales no se pueden calcular
    $chartData['monthly_revenue'] = [];

    // Top modelos más rentados
    $stmt = $dbh->query("
        SELECT 
            m.NombreModelo as Model, 
            COUNT(b.id) as rental_count
        FROM tblbooking b
        JOIN tblmodels m ON b.IdVehiculo = m.id
        GROUP BY b.IdVehiculo, m.NombreModelo
