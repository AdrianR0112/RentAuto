...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  if (empty($username) || empty($password)) {
    ...
    try {
      ...
      $stmt = $dbh->prepare("SELECT id, NombreUsuario, Contrasena FROM admin WHERE NombreUsuario = :username");
      ...
      if ($admin && password_verify($password, $admin['Contrasena'])) {
        ...
        $_SESSION['admin_username'] = $admin['NombreUsuario'];
...
<head>
  <meta charset="utf-8">
  <title>ECUA CARS - Inicio de Sesión Administrador</title>
  ...
  <div class="container h-100">
    <div class="d-flex justify-content-center h-100">
      <div class="user_card">
        ...
        <div class="d-flex justify-content-center">
          <h3 class="text-admin-login">Panel del Administrador</h3>
        </div>
          ...
          <form method="POST" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>">
            <div class="input-group mb-3">
              ...
              <input type="text" name="username" class="form-control input_user"
                value="<?php echo isset($username) ? htmlspecialchars($username) : ''; ?>" placeholder="Usuario"
                required>
            </div>
